...
class attendance_importer {
    ...
    /** @var boolean $incompatsesstime checks scantime column for records with scan times that do not fall within the session time. */
    ...
    /** @var boolean $scantimeerr triggers if the file is suspected of being affected by DST or the wrong file was uploaded for the particular session time. */
    ...
    public function init() {
        ...
        while (!feof($mainfile)) { // Copy the contents of the main file to a temp file.
        ...
        while (!feof($tempfile)) { // Copy the contents of the temp file to below the headings of the main file.
        ...
        while ($record = $this->csvreader->next()){
            ...
            if (!($record[$this->scantimeindex] == date('g:i:s A',$t) ||
                ...
                $this->scantimeformaterr = true; //make a string for this in attendance
            }
