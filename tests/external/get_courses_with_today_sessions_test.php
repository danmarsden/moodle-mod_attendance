...
final class get_courses_with_today_sessions_test extends base_external_testcase {
    /**
     * Tests retrieving courses with today sessions for a valid teacher.
     *
    ...
    /**
     * Tests that an unknown user id triggers a missing-record exception.
     *
     * @covers \mod_attendance\external\get_courses_with_today_sessions::execute
     */
    ...
    /**
     * Tests that cross-user access without capability returns no sessions.
     *
     * @covers \mod_attendance\external\get_courses_with_today_sessions::execute
     */
    public function test_execute_denies_cross_user_without_capability(): void {
        $this->resetAfterTest(true);
    ...
    /**
     * Tests that cross-user access is allowed when capability is present.
     *
     * @covers \mod_attendance\external\get_courses_with_today_sessions::execute
     */
    public function test_execute_allows_cross_user_with_capability(): void {
        $this->resetAfterTest(true);

        $context = $this->create_attendance_context(2);
        $this->setUser($context['teacher']);
        $this->create_session($context['attendance']);
        $otherteacher = $this->getDataGenerator()->create_and_enrol($context['course'], 'editingteacher');
    ...
    /**
     * Tests that userid zero defaults to the current user.
     *
     * @covers \mod_attendance\external\get_courses_with_today_sessions::execute
     */
    public function test_execute_defaults_to_current_user_when_zero(): void {
        $this->resetAfterTest(true);
        ...
        $result = get_courses_with_today_sessions::execute(0);
        $result = external_api::clean_returnvalue(get_courses_with_today_sessions::execute_returns(), $result);
        $this->assertNotEmpty($result);
    }
}
