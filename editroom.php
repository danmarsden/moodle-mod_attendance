...
else $room = (object)['id'=>$id, 'name'=>'', 'description'=>'', 'capacity'=>'', 'bookable'=>1];
...
if ($mform->is_cancelled()) {
    ...
    try {
        ...
        if(!$room->name) throw new InvalidArgumentException('invalid argument!');
        if($room->id) {
            $DB->update_record('attendance_rooms', $room);
            $message = get_string("editroom_success", "mod_attendance", $room->name);
        }
        else {
            $DB->insert_record('attendance_rooms', $room);
            $message = get_string("addroom_success", "mod_attendance", $room->name);
        }
...
$title = $id ? get_string('editroom', 'mod_attendance')
    : get_string('addroom', 'mod_attendance');
