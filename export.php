...
if ($formdata = $mform->get_data()) {
    ...
    if ($reportdata->users) {
        ...
        foreach ($reportdata->users as $user) {
            ...
            $data->table[$i][] = $reportdata->numtakensessions[$user->id];

            $data->table[$i][] = attendance_format_float($reportdata->points[$user->id]) . ' / ' .
                                    attendance_format_float($reportdata->maxpoints[$user->id]);

            $percent = attendance_calc_fraction($reportdata->points[$user->id], $reportdata->maxpoints[$user->id]) * 100;
            $data->table[$i][] = attendance_format_float($percent, false);
